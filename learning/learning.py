...
from learning.experience_batcher import ExperienceBatcher
...
def run_training(train_dir):
    ...
    with tf.compat.v1.Graph().as_default():
        ...
        summary_writer = tf.compat.v1.summary.FileWriter(train_dir, graph_def=session.graph_def, flush_secs=10)

        if resume:
            print("Resuming: ", train_dir)
            saver.restore(session, tf.compat.v1.train.latest_checkpoint(train_dir))
